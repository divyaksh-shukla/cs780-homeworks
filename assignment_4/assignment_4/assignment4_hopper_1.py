...
class DDPG():
    def __init__(self, env, seed, gamma, tau, buffer_size, batch_size, update_frequency, epochs, 
        ...
        self.targetPolicyNetwork = createPolicyNetwork(self.stateDim, self.actionDim, hDim=policyHdim, activation=lambda x: torch.tanh(x))
        self.onlinePolicyNetwork = createPolicyNetwork(self.stateDim, self.actionDim, hDim=policyHdim, activation=lambda x: torch.tanh(x))
...
IMAGES_DIR = os.path.join(CWD, 'images', 'Hopper-v4', 'v_1')
os.makedirs(IMAGES_DIR, exist_ok=True)
...
plotResults(trainRewards, evalRewards, trainTime, wallClockTime, episodeSteps, ["DDPG", "TD3"], "Hopper-v4-v_1")
